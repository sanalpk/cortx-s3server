...
void S3GetBucketVersionAction::get_next_object_version_successful() {
  ...
  for (auto& kv : kvps) {
    ...
    s3_log(S3_LOG_DEBUG, request_id, "Read Object key of std::pair = %i\n",
           kv.second.first);
    ...
    if (last_key_in_common_prefix) {
      s3_log(S3_LOG_DEBUG, request_id, "last_key_in_common_prefix is TRUE\n");
      // Filter by prefix, if prefix specified
      if (!request_prefix.empty()) {
        s3_log(S3_LOG_DEBUG, request_id, "request_prefix is NOT empty\n");
        // Filter out by prefix
        if (kv.first.find(request_prefix) == std::string::npos) {
          s3_log(S3_LOG_DEBUG, request_id,
                 "request_prefix NOT found in kv.first\n");
          ...
          if (kv.first > request_prefix) {
            s3_log(S3_LOG_DEBUG, request_id, "kv.first > request_prefix\n");
          ...
          if (--length == 0) {
            s3_log(S3_LOG_DEBUG, request_id,
                   "length == 0 so exiting the loop\n");
            break;
          } else {
            // Check the next key
            s3_log(S3_LOG_DEBUG, request_id, "continue; to check the next key");
            continue;
          }
        }
      }
      ...
      s3_log(S3_LOG_DEBUG, request_id,
             "last_common_prefix=%s, common_prefix_pos=%zu\n",
             last_common_prefix.c_str(), common_prefix_pos);
      if (common_prefix_pos == std::string::npos) {
        s3_log(S3_LOG_DEBUG, request_id,
               "common_prefix_pos == std::string::npos\n");
        ...
        --length;
        last_key = kv.first;
        s3_log(S3_LOG_DEBUG, request_id, "0000012\n");
        continue;
      }
    }
    ...
    if (request_prefix.empty() && request_delimiter.empty()) {
      s3_log(S3_LOG_DEBUG, request_id, "0000013\n");
      if (object->from_json(kv.second.second) != 0) {
        s3_log(S3_LOG_DEBUG, request_id, "0000014\n");
        ...
        s3_log(S3_LOG_DEBUG, request_id, "Object added to object_list\n");
      }
      ...
      if (kv.first.find(request_prefix) == 0) {
        if (object->from_json(kv.second.second) != 0) {
          ...
          object_list->add_object(object);
          s3_log(S3_LOG_DEBUG, request_id, "Object added to object_list:2\n");
        }
      } else {
        // Prefix does not match.
        // Check if fetched key is lexicographically greater than prefix
      ...
      if (delimiter_pos == std::string::npos) {
        if (object->from_json(kv.second.second) != 0) {
          ...
          s3_log(S3_LOG_DEBUG, request_id, "Object added to object_list:3\n");
          object_list->add_object(object);
        }
      } else {
        // Roll up
        ...
        s3_log(S3_LOG_DEBUG, request_id, "000017\n");
        s3_log(S3_LOG_DEBUG, request_id,
        ...
        if (common_prefix != request_marker_key) {
          ...
          s3_log(S3_LOG_DEBUG, request_id, "000018\n");
        }
      ...
      if (prefix_match) {
        ...
        if (delimiter_pos == std::string::npos) {
          if (object->from_json(kv.second.second) != 0) {
            ...
            object_list->add_object(object);
            s3_log(S3_LOG_DEBUG, request_id,
                   "Object added to objectlist:4 000017\n");
          }
        } else {
          s3_log(S3_LOG_DEBUG, request_id, "000019\n");
          // Roll up
          // All keys under a common prefix are counted as a single key
          // When we encounter a key that belongs to common prefix, the code
          ...
          if (common_prefix != request_marker_key) {
            s3_log(S3_LOG_DEBUG, request_id, "000020\n");
            // If marker is specified, and if this key gets rolled up
            // in common prefix, we add to common prefix only if it is not the
            // same as specified marker
            object_list->add_common_prefix(common_prefix);
        ...
        s3_log(S3_LOG_DEBUG, request_id, "000021\n");
        // Prefix does not match.
        // Check if fetched key is lexicographically greater than prefix
        if (kv.first > request_prefix) {
          // No further prefix match will occur (as items in Motr storage are
    ...
    if ((--length == 0) ||
        (!last_key_in_common_prefix &&
         ((object_list->size() + object_list->common_prefixes_size()) ==
          max_keys))) {
      s3_log(S3_LOG_DEBUG, request_id, "000022\n");
  ...
  if (atleast_one_json_error) {
    s3_log(S3_LOG_DEBUG, request_id, "000023\n");
  ...
  if ((key_Count == max_keys) ||
    ...
    s3_log(S3_LOG_DEBUG, request_id, "000025\n");
    // Go ahead and respond.
    if (key_Count == max_keys && length != 0) {
      ...
      if (b_skip_remaining_common_prefixes) {
        s3_log(S3_LOG_DEBUG, request_id, "000026\n");
